...
def Slider_printer(slider_RI1, slider_RI2, slider_square_x, slider_square_y, slider_laser_x, slider_laser_angle):
    ...
    slider_laser_angle.blit_header()
...
def Square_function():
    ...
    while(x < screen_width):
        ...
        #        laser_thickness = 0
        #        rx = 0
        #        laser_color = colors(slider_RI1.real_value)
#
        #        ry = (y_laser_increment * (slider_square_x.real_value - 75) / x_laser_increment)
        #        pygame.draw.line(screen, 'blue', (0, ry), (slider_square_x.real_value - 75, ry), 5)
#
        #        if slider_laser_pos.real_value > ry:
        #            rx = -2*(slider_laser_pos.real_value - )
#
        #        if slider_laser_pos.real_value < ry:
        #            rx = 2*(ry - slider_laser_pos.real_value)
#
        #        pygame.draw.line(screen, 'red', (slider_square_x.real_value-75, ry), (25, rx), 5)
            ...
            if slider_laser_pos.real_value > (slider_square_y.real_value):
                delta = slider_laser_pos.real_value - (slider_square_y.real_value)
                distance = slider_laser_pos.real_value + 2*delta
            
            if slider_laser_pos.real_value < (slider_square_y.real_value):
                delta = (slider_square_y.real_value) - slider_laser_pos.real_value 
                distance = slider_laser_pos.real_value + 2*delta

            pygame.draw.line(screen, 'blue', (0, 0), (slider_square_x.real_value-75, distance), 5)
...
while not done:
    ...
    for slider in all_sliders:
        slider.blit_header()  # Adjust the alignment as needed
    Square_function()
    reset_button.draw()
    preset1_button.draw()
