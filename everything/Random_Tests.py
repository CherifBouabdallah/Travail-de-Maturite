...
def Calculation():
    ...
    x_square_delta_increment_in = 0
    y_square_delta_increment_in = 0

    pre_calculation_in = ((slider_RI1.real_value * math.sin(math.radians(round(slider_laser_angle.real_value, 2)-180))) / slider_RI2.real_value)
    ...
    if -1 <= pre_calculation_in <= 1:
        ...
        if slider_laser_angle.real_value - 180 > 0:
            x_square_delta_increment_in = x_square_increment_in - x_laser_increment
            y_square_delta_increment_in = -(y_square_increment_in - y_laser_increment)
            
        if slider_laser_angle.real_value - 180 < 0:
            x_square_delta_increment_in = x_square_increment_in
            y_square_delta_increment_in = y_square_increment_in
        
        if slider_laser_angle.real_value - 180 == 0:
            x_square_delta_increment_in = 1



    else:
        x_square_delta_increment_in = -round(math.sin(math.radians(slider_laser_angle.real_value-180)), 2) 
        y_square_delta_increment_in = -round(math.cos(math.radians(slider_laser_angle.real_value-180)), 2)
        refraction = False
...
def Square_function():
    ...
    while(x < screen_width):
        #print(abs(slider_laser_angle.real_value - 180), y_square_delta_increment_in, x_square_delta_increment_in, x_laser_increment, y_laser_increment)

        if slider_RI1.real_value == 0 and slider_RI2.real_value == 0:
